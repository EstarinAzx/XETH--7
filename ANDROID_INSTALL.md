# XETH--7 on Android (Termux)

A complete guide to running XETH--7 on Android using Termux + proot Ubuntu.
...
XETH--7 requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
```bash
cd /data/data/com.termux/files/home/XETH--7
bun run build
```

You should see:
```
✓ Built xeth7 v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to XETH--7's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the XETH--7 repo.
