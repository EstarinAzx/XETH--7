...
export function buildProfileSaveMessage(
  ...
  if (options?.activatedInSession) {
    lines.push('XETH--7 switched to it for this session.')
    ...
    lines.push('Restart XETH--7 to use it.')
  }
...
function ProviderChooser({
  ...
  const helperText = canUseCodexOAuth
    ? 'Save a provider profile without editing environment variables first. Codex profiles backed by env, auth.json, or XETH--7 secure storage can switch this session immediately when validation succeeds.'
...
function CodexOAuthStep({
  ...
  const handleAuthenticated = React.useCallback(async (
    ...
    if (!env) {
      throw new Error(
        'Codex OAuth succeeded, but XETH--7 could not build a Codex profile from the stored credentials.',
      )
    }
  ...
  return (
    <Dialog title="Codex OAuth" onCancel={onBack}>
      <Box flexDirection="column" gap={1}>
        <Text>
          Finish signing in with ChatGPT in your browser. XETH--7 will store
        ...
        {status.browserOpened === false ? (
          ...
          <Text dimColor>
            Browser opened. Complete the sign-in there, then XETH--7 will
        ...
        <ProviderChooser
          onChoose={value => {
            if (value === 'auto') {
              ...
              onDone(`Removed saved provider profile at ${filePath}. Restart XETH--7 to go back to normal startup.`, {
                display: 'system',
              })
            } else {
