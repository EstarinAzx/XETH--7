  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading Stratagem sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your Stratagem sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading Stratagem sessions
        </Text>
...
function renderErrorSpecificGuidance(errorType: LoadErrorType): React.ReactNode {
  switch (errorType) {
    ...
    case 'api':
      return <Box marginY={1} flexDirection="column">
          <Text dimColor>Sorry, Stratagem encountered an error</Text>
        </Box>;
    case 'other':
