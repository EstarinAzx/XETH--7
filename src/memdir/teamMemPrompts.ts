...
export function buildCombinedMemoryPrompt(
  ...
  const howToSave = skipIndex
    ? [
        ...
        '',
        'Write each durable memory as its own knowledge note under the chosen directory\'s `knowledge/` subtree (private or team, per the type\'s scope guidance) using this frontmatter format:',
        '',
        ...MEMORY_FRONTMATTER_EXAMPLE,
        '',
        '- Update the matching `index.md` with a one-line wikilink to the note',
        '- Append a short bullet to today\'s `daily/YYYY-MM-DD.md` log in the same memory root',
    ...
    : [
        ...
        '',
        "**Step 1** — write the durable knowledge note to the best matching folder under the chosen directory's `knowledge/` subtree (private or team, per the type's scope guidance) using this frontmatter format:",
        '',
        ...MEMORY_FRONTMATTER_EXAMPLE,
        '',
        `**Step 2** — add a pointer to that note in the same directory's \`${ENTRYPOINT_NAME}\`. Each directory (private and team) has its own \`${ENTRYPOINT_NAME}\` index — each entry should be one line, under ~150 characters, and should use a wikilink such as \`- [[knowledge/connections/order-payment-flow]] — ties checkout failures to payment retries\`. Never write full memory content directly into an index.`,
        '',
        '**Step 3** — append a brief note to today\'s `daily/YYYY-MM-DD.md` log in the same memory root.',
        '',
